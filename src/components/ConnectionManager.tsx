...
export function ConnectionManager({
  ...
  function updateDraft(updates: Partial<ConnectionProfile>) {
    setDraft((d) => {
      ...
      // Auto-update port when type changes (unless port was explicitly set)
  ...
  function handleNameChange(value: string) {
    if (isConnectionUrl(value)) {
      ...
      setDraft((d) => ({ ...d, ...parsed }));
      setStatusMsg(null);
      setStatusMsg({ type: "url", text: "Connection URL parsed — fields populated below" });
    } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="flex w-[740px] h-[500px] rounded-xl overflow-hidden bg-bg-secondary border border-border shadow-2xl">
